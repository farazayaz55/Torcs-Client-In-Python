...
columns=['angle','curLapTime','damage','distFromStart','distRaced','fuel','gear','lastLapTime','opponents','racePos','rpm','speedX','speedY','speedZ','track','wheelSpinVel','z','focus1','accel','brake','clutch','gear','steer','focus','meta']
noOfcolumns=len(columns)
listoflists=[]
class Client():
    global columns,noOfcolumns,listoflists
    ...
    def setup_connection(self):
        ...
        while True:
            ...
            identify = '***identified***'
            if identify in sockdata.decode():
    ...
    def get_servers_input(self):
        ...
        while True:
            try:
                # Receive server data
                sockdata,addr= self.so.recvfrom(data_size)
                if(isinstance(sockdata,bytes)):
                    self.serv=sockdata.decode()
                else:
                    self.serv=sockdata
                #print(sockdata)
                #sockdata = sockdata.decode('utf-8')
    ...
    #it basically converts server message to dataframe so that it becomes our test set now we have our whole csv file which we can use as our training data
    def KNNUSE(self):
        a=self.splitt(self.serv,"angle")
        lista=[float(a)]
        c=self.splitt(self.serv,"curLapTime")
        listb=[float(c)]
        d=self.splitt(self.serv,"damage")
        listc=[float(d)]
        d=self.splitt(self.serv,"distFromStart")
        listd=[float(d)]
        e=self.splitt(self.serv,"distRaced")
        liste=[float(e)]
        f=self.splitt(self.serv,"fuel")
        listf=[float(f)]
        g=self.splitt(self.serv,"gear")
        listg=[float(g)]
        h=self.splitt(self.serv,"lastLapTime")
        listh=[float(h)]
        i=self.splitt(self.serv,"opponents")
        listi=self.flolist(i)
        j=self.splitt(self.serv,"racePos")
        listj=[float(j)]
        k=self.splitt(self.serv,"rpm")
        listk=[float(k)]
        l=self.splitt(self.serv,"speedX")
        listl=[float(l)]
        m=self.splitt(self.serv,"speedY")
        listm=[float(m)]
        n=self.splitt(self.serv,"speedZ")
        listn=[float(n)]
        o=self.splitt(self.serv,"track")
        listo=self.flolist(o)
        p=self.splitt(self.serv,"trackPos")
        listp=[float(p)]
        q=self.splitt(self.serv,"wheelSpinVel")
        listq=self.flolist(q)
        r=self.splitt(self.serv,"z")
        listr=[float(r)]
        s=self.splitt(self.serv,"focus")
        lists=self.flolist(s)    
        #these all are float features
        X_test = pd.DataFrame(list(zip(lista,listb,listc,listd,liste,listf,listg,listh,listi,listj,listk,listl,listm,listn,listo,listp,listq,listr,lists)))
    ...
    def KNN_ACCURACY(self):
        #through excel standardized data
        #load data
        #find neighbours
        
        df=pd.read_csv('dataset2.csv',header=None)
        from sklearn.model_selection import train_test_split
        X_train,X_test,Y_Train,Y_Test = train_test_split(df.iloc[:,0:19],df.iloc[:,19:24], test_size = 0.2)#we have 15 features and 5 labels
    ...
    def splitt(self,bigString,Value):
            start=bigString.find(Value)
            offset=len(Value)
            end=bigString.find(")",start)
    
            return bigString[start+offset+1:end]
    

    def flolist(self,stri):#convert opponenets string to float array:
        float_list=[]
    ...
    def respond_to_server(self):
        ...
        try:
            ...
            if(isinstance(message,bytes)):
                self.cli=message.decode()
            else:
                self.cli=message
            '''
            #total_row=self.serv+self.cli
            a=self.splitt(self.serv,"angle")
            lista=[float(a)]
            c=self.splitt(self.serv,"curLapTime")
            listb=[float(c)]
            ...
            s=self.splitt(self.serv,"focus")
            lists=self.flolist(s)
            list2=[self.tobool(float(W))]
            list3=[self.tobool(float(X))]
            list4=[self.tobool(float(F))]
            list5=[self.tobool(float(A))]
            list6=[self.tobool(float(D))]
            #these all are strings
            df = pd.DataFrame(list(zip(lista,listb,listc,listd,liste,listf,listg,listh,listi,listj,listk,listl,listm,listn,listo,listp,listq,listr,lists, list2,list3,list4,list5,list6)))
            #columns =['angle','curLapTime','damage','distFromStart','distRaced','fuel','gear','lastLapTime','opponents','racePos','rpm','speedX','speedY','speedZ','track','W','X','F','A','D']
            df.to_csv('dataset2.csv',mode='a',header=None,index=False)#to make an empty file remove header =false run ccode for sometime use excel to remove duplicate then add header=False
...
class DriverAction():
    ...
    '''accel is +, breake is pos+,steer is both'''
    def __init__(self):
       self.actionstr= str()
...
def X_key(c):
    ...
    target_speed=100
    global W,X,F,A,D
    if S['speedX'] < target_speed - (R['steer']*50)==False:
        R['accel']-= .01
        X="1"
        W="0"
def A_key(c):
    ...
    R['steer']=((S['angle']- S['trackPos']*0.5)/0.785398)
def D_key(c):
    S,R= c.S.d,c.R.d
    target_speed=100
    global W,X,F,A,D
